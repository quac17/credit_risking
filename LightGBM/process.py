...
def prepare_data(df):
    ...
    # Fix categorical columns for LightGBM
...
def evaluate_and_save(model, df, name, threshold, has_target=True):
    ...
    result_path = os.path.join(OUTPUT_DIR, f'LightGBM_result_{name}.csv')
...
def run_pipeline():
    ...
    plt.title('LightGBM Multi-Dataset Performance')
    ...
    print(f"LightGBM pipeline complete. Results in {OUTPUT_DIR}")
